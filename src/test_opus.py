...
from utils import dump_json, dump_jsonl, load_jsonl
...
class TestDataset(Dataset):
    ...
    def get_features(self, examples):
        ...
        zh = self.tokenizer(zh_lines, **kwargs)
        en = self.tokenizer(en_lines, **kwargs)
    ...
    def __getitem__(self, idx):
        return {k: LongTensor(v) for k, v in self.features[idx].items()}
...
def test_all(trainer, tokenizer):
    for test_phase in ['clean', 'noisy_1', 'noisy_2', 'noisy_3']:
        ...
        examples_file = f'../data/keyboard/nmt/nmt_test_{test_phase}.json'
        print(f'Getting from {examples_file}')
        dataset = TestDataset(examples_file, tokenizer)
